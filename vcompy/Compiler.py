...
class Compiler:
	...
	def save_as(self, filename):
		...
		while self.frameIndex < duration:
			...
			for clipIndex, clip in clipsInFrame:
				clipType = type(clip)
...
#					print(f"[{self.frameIndex}], Cached: {not notcached}")
					if notcached:
						...
						# Insert to clips
						self.clips.insert(clipIndex, subclip)
...
#						print(f"{clip.clip_start} <{clip.start}>---------<{clip.start + clip.duration}>")
			...
			for cache in self.pagecache:
				# TODO: Tidy this
				if not cache in map(lambda t: t[1], clipsInFrame) and not cache in newCache:
